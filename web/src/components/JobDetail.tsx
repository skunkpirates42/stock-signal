...
function metric(metric: Metric | undefined): string {
  ...
  return metric.unit.startsWith("USD") ? formatCurrency(metric.observation.value) : String(metric.observation.value);
}
...
export default function JobDetail({ detail, warnings }: { detail: Detail; warnings: string[] }) {
  ...
  return <main>
    ...
    <section className="card"><h2>Exact queued configuration</h2><dl className="config-grid">
      ...
      <div><dt>Window</dt><dd>{run.window.name}: {formatTimestamp(run.window.start)} inclusive to {formatTimestamp(run.window.end_exclusive)} exclusive</dd></div>
      ...
      <div><dt>Accounting</dt><dd>{run.accounting.coverage}; unresolved: {run.accounting.unresolved.join(", ") || "none declared"}</dd></div>
    ...
    <section className="card"><h2>Result</h2>
      {result ? <><p className="card-note">Python saved these metrics under its declared basis. Missing amounts remain unavailable.</p>
        <dl className="metric-grid">{Object.entries(result.metrics).map(([name, value]) =>
          <div key={name}><dt>{name.replaceAll("_", " ")}</dt><dd className="num">{metric(value)}</dd><small>{value.basis}</small></div>)}</dl>
