...
function detail(state: "failed" | "completed"): Detail {
  return {
    run: { id: RUN_ID, strategy_id: "stock-signal", strategy_version: available("v1"), dataset_id: "fixture",
      ...
      window: { name: "fixture-day-2", start: "2026-09-24T12:00:00Z", end_exclusive: "2026-09-24T13:00:00Z" },
      provenance: { synthetic: true, retrospective: false }, feed: available("synthetic"),
      cost_policy: { id: "base-v2", spread_bps: 2, slippage_bps_per_fill: 1, fee_per_share_per_fill: 0, evidence: "Scenario" },
      accounting: { coverage: "incomplete", unresolved: ["borrow_cost"] }, code: { revision: unavailable("No revision"), source_sha256: available("source") },
    ...
    result: state === "completed" ? { id: "result-id", metrics: { realized_net_pnl: { observation: available(12), unit: "USD", basis: "Saved Python result" } },
